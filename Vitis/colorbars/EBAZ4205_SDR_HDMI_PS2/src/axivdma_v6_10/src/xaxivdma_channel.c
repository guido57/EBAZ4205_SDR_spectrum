 ...
 */
static u32 XAxiVdma_BdRead(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Offset);
static void XAxiVdma_BdWrite(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Offset, u32 Value);
static void XAxiVdma_BdSetNextPtr(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, u32 NextPtr);
static void XAxiVdma_BdSetAddr(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, u32 Addr);
static int XAxiVdma_BdSetVsize(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Vsize);
static int XAxiVdma_BdSetHsize(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Vsize);
static int XAxiVdma_BdSetStride(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Stride);
static int XAxiVdma_BdSetFrmDly(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int FrmDly);
 ...
 *
 *****************************************************************************/
#define XAxiVdma_ChannelHiFrmAddrEnable(_mm_IP, Channel) \
{ \
	XAxiVdma_WriteReg(_mm_IP,Channel->ChanBase, \
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelInit(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	int i;
	int NumFrames;
	...
	for (i = 0; i < NumFrames; i++) {
		...
		XAxiVdma_BdSetNextPtr(_mm_IP, BdPtr,
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelResetNotDone(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	return (XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CR_OFFSET) &
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelReset(mm_IP * _mm_IP,XAxiVdma_Channel *Channel)
{
	XAxiVdma_WriteReg(_mm_IP, Channel->ChanBase, XAXIVDMA_CR_OFFSET,
	    XAXIVDMA_CR_RESET_MASK);
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelIsRunning(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 Bits;

	/* If halted bit set, channel is not running
	 */
	Bits = XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase, XAXIVDMA_SR_OFFSET) &
	 ...
	 */
	Bits = XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase, XAXIVDMA_CR_OFFSET) &
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelIsBusy(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 Bits;
 ...
 *
 *****************************************************************************/
u32 XAxiVdma_ChannelErrors(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	return (XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase, XAXIVDMA_SR_OFFSET)
 ...
 *
 *****************************************************************************/
void XAxiVdma_ClearChannelErrors(mm_IP * _mm_IP,XAxiVdma_Channel *Channel, u32 ErrorMask)
{
	u32 SrBits;

	/* Write on Clear bits */
        SrBits = XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase, XAXIVDMA_SR_OFFSET)
                        | ErrorMask;

	XAxiVdma_WriteReg(_mm_IP, Channel->ChanBase, XAXIVDMA_SR_OFFSET,
	    SrBits);
 ...
 *
 *****************************************************************************/
u32 XAxiVdma_ChannelGetStatus(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	return XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_SR_OFFSET);
}
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelStartParking(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 CrBits;

	if (!XAxiVdma_ChannelIsRunning(_mm_IP, Channel)) {
	...
	CrBits = XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CR_OFFSET) &
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelStopParking(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 CrBits;
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelStartFrmCntEnable(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 CrBits;
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelSetBdAddrs(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, UINTPTR BdAddrPhys,
		UINTPTR BdAddrVirt)
{
	...
	if (Channel->HasSG && XAxiVdma_ChannelIsBusy(_mm_IP, Channel)) {
		xdbg_printf(XDBG_DEBUG_ERROR,
	...
	for (i = 0; i < NumFrames; i++) {
		...
		XAxiVdma_BdSetNextPtr(_mm_IP, BdPtr, NextPhys);
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelStartTransfer(mm_IP * _mm_IP, XAxiVdma_Channel *Channel,
    XAxiVdma_ChannelSetup *ChannelCfgPtr)
{
	...
	Status = XAxiVdma_ChannelConfig(_mm_IP, Channel, ChannelCfgPtr);
	...
	Status = XAxiVdma_ChannelSetBufferAddr(_mm_IP, Channel,
	...
	Status = XAxiVdma_ChannelStart(_mm_IP, Channel);
	if (Status != XST_SUCCESS) {
		xdbg_printf(XDBG_DEBUG_ERROR,
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelConfig(mm_IP * _mm_IP, XAxiVdma_Channel *Channel,
        XAxiVdma_ChannelSetup *ChannelCfgPtr)
{
	u32 CrBits;
	int i;
	...
	CrBits = XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase,
	     XAXIVDMA_CR_OFFSET);
	...
	if (ChannelCfgPtr->EnableCircularBuf) {
		...
		if (Channel->IsRead) {
			...
			RegValue = XAxiVdma_ReadReg(_mm_IP, Channel->InstanceBase,
			...
			XAxiVdma_WriteReg(_mm_IP,Channel->InstanceBase,
	...
	if (Channel->HasVFlip && !Channel->IsRead) {
		...
		XAxiVdma_WriteReg(_mm_IP,Channel->InstanceBase, XAXIVDMA_VFLIP_OFFSET,
			RegValue);
	}
	...
	if (Channel->HasSG) {
		...
		for (i = 0; i < NumBds; i++) {
			...
			Status = XAxiVdma_BdSetVsize(_mm_IP, BdPtr,
			...
			Status = XAxiVdma_BdSetHsize(_mm_IP, BdPtr,
			...
			Status = XAxiVdma_BdSetStride(_mm_IP, BdPtr,
			...
			Status = XAxiVdma_BdSetFrmDly(_mm_IP, BdPtr,
		...
		//GG XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
		//GG    XAXIVDMA_HSIZE_OFFSET, ChannelCfgPtr->HoriSizeInput);
		XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
		    0x54 , ChannelCfgPtr->HoriSizeInput); // Horiz Size is at offset 0x54

		//GG XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
		//GG		    XAXIVDMA_STRD_FRMDLY_OFFSET,
		//GG		    (ChannelCfgPtr->FrameDelay << XAXIVDMA_FRMDLY_SHIFT) |
		//GG 		    ChannelCfgPtr->Stride);
		XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
		    0x58,			// frame delay and stride register is at offset 0x58
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelSetBufferAddr(mm_IP * _mm_IP, XAxiVdma_Channel *Channel,
        UINTPTR *BufferAddrSet, int NumFrames)
{
	...
	for (i = 0; i < NumFrames; i++, Loop16++) {
		...
		if (Channel->HasSG) {
			XAxiVdma_BdSetAddr(_mm_IP, BdPtr, BufferAddrSet[i]);
		}
		else {
			if ((i > FrmBound) && !HiFrmAddr) {
				XAxiVdma_ChannelHiFrmAddrEnable(_mm_IP, Channel);
				HiFrmAddr = 1;
				Loop16 = 0;
			}
...
//GG				XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
//GG					XAXIVDMA_START_ADDR_OFFSET +
//GG					Loop16 * XAXIVDMA_START_ADDR_LEN,
//GG					BufferAddrSet[i]);
				XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase, //GG StartAddrBase is not used in XAxiVdma_WriteReg
					0x5c +    									 //GG The first buffer address is at 0x5c
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelStart(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 CrBits;
	...
	if (!XAxiVdma_ChannelIsRunning(_mm_IP, Channel)) {
		...
		if (Channel->HasSG) {
			 ...
			 */
			XAxiVdma_WriteReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CDESC_OFFSET,
	...
	if (XAxiVdma_ChannelIsRunning(_mm_IP, Channel)) {
		...
		if (Channel->HasSG) {
			 ...
			 * fetching BDs
			 */
			XAxiVdma_WriteReg(_mm_IP,Channel->ChanBase, XAXIVDMA_TDESC_OFFSET,
			 ...
			 */
			//GG XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase,
			//GG    XAXIVDMA_VSIZE_OFFSET, Channel->Vsize);

			XAxiVdma_WriteReg(_mm_IP,Channel->StartAddrBase, // WriteReg doesn't use the BaseAddress
			    0x50, Channel->Vsize); //GG VSIZE is at offset 0x50


		}

		//GG Check again if channel is running
		if (XAxiVdma_ChannelIsRunning(_mm_IP, Channel)) {
			int a = 3;
		}
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelStop(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	u32 CrBits;
	...
	XAxiVdma_WriteReg(_mm_IP, Channel->ChanBase, XAXIVDMA_CR_OFFSET, CrBits);
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelRegisterDump(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	printf("Dump register for channel %p:\r\n", (void *)Channel->ChanBase);
	xil_printf("\tControl Reg: %x\r\n",
	    XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CR_OFFSET));
	xil_printf("\tStatus Reg: %x\r\n",
	    XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_SR_OFFSET));
	xil_printf("\tCDESC Reg: %x\r\n",
	    XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CDESC_OFFSET));
	xil_printf("\tTDESC Reg: %x\r\n",
	    XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_TDESC_OFFSET));
 ...
 *
 *****************************************************************************/
int XAxiVdma_ChannelSetFrmCnt(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, u8 FrmCnt, u8 DlyCnt)
{
	u32 CrBits;
 ...
 *****************************************************************************/
void XAxiVdma_ChannelGetFrmCnt(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, u8 *FrmCnt,
        u8 *DlyCnt)
{
	u32 CrBits;
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelEnableIntr(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, u32 IntrType)
{
	u32 CrBits;
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelDisableIntr(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, u32 IntrType)
{
	u32 CrBits;
	u32 IrqBits;
 ...
 *
 *****************************************************************************/
u32 XAxiVdma_ChannelGetPendingIntr(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	return (XAxiVdma_ReadReg(_mm_IP, Channel->ChanBase, XAXIVDMA_SR_OFFSET) &
	          XAXIVDMA_IXR_ALL_MASK);
}
 ...
 *
 *****************************************************************************/
void XAxiVdma_ChannelIntrClear(mm_IP * _mm_IP, XAxiVdma_Channel *Channel, u32 IntrType)
{
 ...
 *
 *****************************************************************************/
u32 XAxiVdma_ChannelGetEnabledIntr(mm_IP * _mm_IP, XAxiVdma_Channel *Channel)
{
	return (XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase, XAXIVDMA_CR_OFFSET) &
	          XAXIVDMA_IXR_ALL_MASK);
}
...
static void XAxiVdma_BdSetNextPtr(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, u32 NextPtr)
{
	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_NDESC_OFFSET, NextPtr);
	return;
}
...
static void XAxiVdma_BdSetAddr(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, u32 Addr)
{
	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_START_ADDR_OFFSET, Addr);
...
static int XAxiVdma_BdSetVsize(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Vsize)
{
	...
	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_VSIZE_OFFSET, Vsize);
	return XST_SUCCESS;
}
 ...
 *
 *****************************************************************************/
static int XAxiVdma_BdSetHsize(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Hsize)
{
	...
	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_HSIZE_OFFSET, Hsize);
	return XST_SUCCESS;
}
...
static int XAxiVdma_BdSetStride(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int Stride)
{
	...
	Bits = XAxiVdma_BdRead(_mm_IP, BdPtr, XAXIVDMA_BD_STRIDE_OFFSET) &
	        ~XAXIVDMA_STRIDE_MASK;

	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_STRIDE_OFFSET, Bits | Stride);
...
static int XAxiVdma_BdSetFrmDly(mm_IP * _mm_IP, XAxiVdma_Bd *BdPtr, int FrmDly)
{
	...
	XAxiVdma_BdWrite(_mm_IP, BdPtr, XAXIVDMA_BD_STRIDE_OFFSET,
