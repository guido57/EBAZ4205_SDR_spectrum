 ...
 *****************************************************************************/
XAxiVdma_Channel *XAxiVdma_GetChannel(mm_IP * _mm_IP,XAxiVdma *InstancePtr,
        u16 Direction)
{
...
static int XAxiVdma_Major(mm_IP * _mm_IP, XAxiVdma *InstancePtr) {
	u32 Reg;

	//Reg = XAxiVdma_ReadReg(InstancePtr->BaseAddr, XAXIVDMA_VERSION_OFFSET);
	Reg = IP_driver_read(_mm_IP, XAXIVDMA_VERSION_OFFSET);
 ...
 *****************************************************************************/
int XAxiVdma_CfgInitialize(mm_IP * _mm_IP, XAxiVdma *InstancePtr, XAxiVdma_Config *CfgPtr,
				UINTPTR EffectiveAddr)
{
	...
	if (XAxiVdma_Major(_mm_IP, InstancePtr) < 3) {
		InstancePtr->HasSG = 1;
	}
	else {
	...
	if (InstancePtr->HasMm2S) {
		...
		if(RdChannel->LineBufDepth > 0) {
			RdChannel->LineBufThreshold =
				XAxiVdma_ReadReg(_mm_IP,RdChannel->ChanBase,
		...
		XAxiVdma_ChannelInit(_mm_IP, RdChannel);
	...
	if (InstancePtr->HasS2Mm) {
		...
		if(WrChannel->LineBufDepth > 0) {
			WrChannel->LineBufThreshold =
				XAxiVdma_ReadReg(_mm_IP,WrChannel->ChanBase,
					XAXIVDMA_BUFTHRES_OFFSET);
			xdbg_printf(XDBG_DEBUG_GENERAL,
		...
		XAxiVdma_ChannelInit(_mm_IP, WrChannel);
 ...
 *****************************************************************************/
void XAxiVdma_Reset(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;

	Channel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, Direction);
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelReset(_mm_IP, Channel);
 ...
 *****************************************************************************/
int XAxiVdma_ResetNotDone(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	return XAxiVdma_ChannelResetNotDone(_mm_IP, Channel);
}
 ...
 *
 *****************************************************************************/
int XAxiVdma_IsBusy(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelIsBusy(_mm_IP, Channel);
	}
	else {
 ...
 *****************************************************************************/
u32 XAxiVdma_CurrFrameStore(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction)
{
	u32 Rc;

	Rc = XAxiVdma_ReadReg(_mm_IP,InstancePtr->BaseAddr, XAXIVDMA_PARKPTR_OFFSET);
 ...
 *
 *****************************************************************************/
u32 XAxiVdma_GetVersion(mm_IP * _mm_IP,XAxiVdma *InstancePtr)
{
	return XAxiVdma_ReadReg(_mm_IP,InstancePtr->BaseAddr, XAXIVDMA_VERSION_OFFSET);
}
 ...
 *****************************************************************************/
u32 XAxiVdma_GetStatus(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelGetStatus(_mm_IP, Channel);
	}
	else {
		return 0xFFFFFFFF;
	}
 ...
 *****************************************************************************/
int XAxiVdma_SetLineBufThreshold(mm_IP * _mm_IP,XAxiVdma *InstancePtr, int LineBufThreshold,
	u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if(Channel->LineBufThreshold) {
		if((LineBufThreshold < Channel->LineBufDepth) &&
			...
			xdbg_printf(XDBG_DEBUG_GENERAL,
				...
				XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase,
				XAXIVDMA_BUFTHRES_OFFSET));
		}
		else {
			xdbg_printf(XDBG_DEBUG_ERROR,
 ...
 *
 *****************************************************************************/
int XAxiVdma_FsyncSrcSelect(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 Source,
				u16 Direction)
{
	XAxiVdma_Channel *Channel;
	u32 CrBits;
 ...
 *
 *****************************************************************************/
int XAxiVdma_GenLockSourceSelect(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 Source,
					u16 Direction)
{
	...
	if (InstancePtr->HasMm2S && InstancePtr->HasS2Mm &&
		...
		if (Direction == XAXIVDMA_WRITE) {
			Channel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, Direction);
			XChannel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, XAXIVDMA_READ);
		} else {
			Channel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, Direction);
			XChannel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, XAXIVDMA_WRITE);
		}
 ...
 *****************************************************************************/
int XAxiVdma_StartParking(mm_IP * _mm_IP, XAxiVdma *InstancePtr, int FrameIndex,
         u16 Direction)
{
	XAxiVdma_Channel *Channel;
	u32 FrmBits;
	...
	if (Direction == XAXIVDMA_READ) {
		...
		RegValue = XAxiVdma_ReadReg(_mm_IP,InstancePtr->BaseAddr,
		...
		XAxiVdma_WriteReg(_mm_IP, InstancePtr->BaseAddr,
	...
	if (Channel->IsValid) {
		Status = XAxiVdma_ChannelStartParking(_mm_IP, Channel);
 ...
 *****************************************************************************/
void XAxiVdma_StopParking(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelStopParking(_mm_IP, Channel);
	}
 ...
 *
 *****************************************************************************/
void XAxiVdma_StartFrmCntEnable(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelStartFrmCntEnable(_mm_IP, Channel);
	}
}
 ...
 *****************************************************************************/
int XAxiVdma_SetBdAddrs(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 BdAddrPhys, u32 BdAddrVirt,
         int NumBds, u16 Direction)
{
	...
	if (Channel->IsValid) {
		...
		return XAxiVdma_ChannelSetBdAddrs(_mm_IP, Channel, BdAddrPhys, BdAddrVirt);
	}
	else {
 ...
 *
 *****************************************************************************/
int XAxiVdma_StartWriteFrame(mm_IP * _mm_IP, XAxiVdma *InstancePtr,
    XAxiVdma_DmaSetup *DmaConfigPtr)
{
	XAxiVdma_Channel *Channel;

	Channel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, XAXIVDMA_WRITE);

	if (Channel->IsValid) {
		return XAxiVdma_ChannelStartTransfer(_mm_IP, Channel,
 ...
 *
 *****************************************************************************/
int XAxiVdma_StartReadFrame(mm_IP * _mm_IP, XAxiVdma *InstancePtr,
        XAxiVdma_DmaSetup *DmaConfigPtr)
{
	XAxiVdma_Channel *Channel;

	Channel = XAxiVdma_GetChannel(_mm_IP, InstancePtr, XAXIVDMA_READ);
 ...
 *
 *****************************************************************************/
int XAxiVdma_DmaConfig(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction,
        XAxiVdma_DmaSetup *DmaConfigPtr)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		...
		return XAxiVdma_ChannelConfig(_mm_IP, Channel,
		    (XAxiVdma_ChannelSetup *)DmaConfigPtr);
	}
	else {
		return XST_DEVICE_NOT_FOUND;
 ...
 *
 *****************************************************************************/
int XAxiVdma_DmaSetBufferAddr(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction,
        UINTPTR *BufferAddrSet)
{
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelSetBufferAddr(_mm_IP, Channel, BufferAddrSet,
		    Channel->NumFrames);
	}
	else {
		return XST_DEVICE_NOT_FOUND;
 ...
 *
 *****************************************************************************/
int XAxiVdma_DmaStart(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelStart(_mm_IP,Channel);
	}
	else {
		return XST_DEVICE_NOT_FOUND;
	}
 ...
 *****************************************************************************/
void XAxiVdma_DmaStop(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelStop(_mm_IP, Channel);
	}
 ...
 * If channel is invalid, then do nothing on that channel
 *****************************************************************************/
void XAxiVdma_DmaRegisterDump(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelRegisterDump(_mm_IP, Channel);
	}
 ...
 * If channel is invalid, then do nothing on that channel
 *****************************************************************************/
int XAxiVdma_SetFrameCounter(mm_IP * _mm_IP, XAxiVdma *InstancePtr,
        XAxiVdma_FrameCounter *CfgPtr)
{
	...
	if (Channel->IsValid) {
		...
		Status = XAxiVdma_ChannelSetFrmCnt(_mm_IP, Channel, CfgPtr->ReadFrameCount,
	...
	if (Channel->IsValid) {
		...
		Status = XAxiVdma_ChannelSetFrmCnt(_mm_IP, Channel,
 ...
 *****************************************************************************/
void XAxiVdma_GetFrameCounter(mm_IP * _mm_IP, XAxiVdma *InstancePtr,
        XAxiVdma_FrameCounter *CfgPtr)
{
	XAxiVdma_Channel *Channel;
	u8 FrmCnt;
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelGetFrmCnt(_mm_IP, Channel, &FrmCnt, &DlyCnt);
	}
 ...
 *
 *****************************************************************************/
int XAxiVdma_SetFrmStore(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u8 FrmStoreNum, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	XAxiVdma_WriteReg(_mm_IP, Channel->ChanBase, XAXIVDMA_FRMSTORE_OFFSET,
	    FrmStoreNum & XAXIVDMA_FRMSTORE_MASK);

	Channel->NumFrames = FrmStoreNum;

	XAxiVdma_ChannelInit(_mm_IP, Channel);
 ...
 *
 *****************************************************************************/
void XAxiVdma_GetFrmStore(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u8 *FrmStoreNum,
				u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->DbgFeatureFlags & XAXIVDMA_ENABLE_DBG_FRMSTORE_REG) {
		*FrmStoreNum = (XAxiVdma_ReadReg(_mm_IP,Channel->ChanBase,
 ...
 *
 *****************************************************************************/
int XAxiVdma_GetDmaChannelErrors(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelErrors(_mm_IP, Channel);
	}
	else {
		return XST_DEVICE_NOT_FOUND;
	}
 ...
 *
 *****************************************************************************/
int XAxiVdma_ClearDmaChannelErrors(mm_IP * _mm_IP,XAxiVdma *InstancePtr, u16 Direction,
					u32 ErrorMask)
{
	XAxiVdma_Channel *Channel;
	...
	if (Channel->IsValid) {
		XAxiVdma_ClearChannelErrors(_mm_IP, Channel, ErrorMask);
		return XST_SUCCESS;
	}
	else {
		return XST_DEVICE_NOT_FOUND;
