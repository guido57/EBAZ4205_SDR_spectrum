...
void XAxiVdma_IntrEnable(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 IntrType, u16 Direction)
{
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelEnableIntr(_mm_IP, Channel, IntrType);
	}
...
void XAxiVdma_IntrDisable(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 IntrType, u16 Direction)
{
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelDisableIntr(_mm_IP,Channel, IntrType);
	}
...
u32 XAxiVdma_IntrGetPending(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u16 Direction)
{
	...
	if (Channel->IsValid) {
		return XAxiVdma_ChannelGetPendingIntr(_mm_IP, Channel);
	}
	else {
...
void XAxiVdma_IntrClear(mm_IP * _mm_IP, XAxiVdma *InstancePtr, u32 IntrType, u16 Direction)
{
	...
	if (Channel->IsValid) {
		XAxiVdma_ChannelIntrClear(_mm_IP, Channel, IntrType);
	}
	return;
}
...
void XAxiVdma_ReadIntrHandler(mm_IP * _mm_IP,  void * InstancePtr)
{
	...
	Channel = XAxiVdma_GetChannel(_mm_IP, DmaPtr, XAXIVDMA_READ);
	...
	PendingIntr = XAxiVdma_ChannelGetPendingIntr(_mm_IP, Channel);
	PendingIntr &= XAxiVdma_ChannelGetEnabledIntr(_mm_IP,Channel);

	XAxiVdma_ChannelIntrClear(_mm_IP, Channel, PendingIntr);
...
void XAxiVdma_WriteIntrHandler(mm_IP * _mm_IP, void * InstancePtr)
{
	...
	Channel = XAxiVdma_GetChannel(_mm_IP, DmaPtr, XAXIVDMA_WRITE);
