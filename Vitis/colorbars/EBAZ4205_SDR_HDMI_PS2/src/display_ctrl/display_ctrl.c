...
#include "../xdebug.h"
//#include "xil_io.h"
#include "../vtc_v8_4/src/xvtc.h" //GG
#include "../axivdma_v6_10/src/xaxivdma.h"
#include "../IP_Driver.h"
/* ------------------------------------------------------------ */
...
**
*/
int DisplayStop(mm_IP * _vdma_mm_IP, mm_IP * _vtc_mm_IP, DisplayCtrl *dispPtr)
{
	/*
	 ...
	 * mid-frame)
	 */
	XVtc_DisableGenerator(_vtc_mm_IP, &dispPtr->vtc);

	/*
	 * Stop the VDMA core
	 */
	XAxiVdma_DmaStop(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ);
	while(XAxiVdma_IsBusy(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ));
	...
	if (XAxiVdma_GetDmaChannelErrors(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ))
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Clearing DMA errors...\r\n");
		XAxiVdma_ClearDmaChannelErrors(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ, 0xFFFFFFFF);
...
**
*/
int DisplayStart(mm_IP * _vtc_mm_IP, mm_IP * _vdma_mm_IP, mm_IP * _dynclk_mm_IP, DisplayCtrl *dispPtr)
{
	int Status;
	ClkConfig clkReg;
	ClkMode clkMode;
	 ...
	 */
	ClkFindParams(_dynclk_mm_IP, dispPtr->vMode.freq, &clkMode);
	...
	ClkWriteReg(_dynclk_mm_IP, &clkReg, dispPtr->dynClkAddr);

	/*
	 * Enable the dynamically generated clock
    */
	ClkStop(_dynclk_mm_IP, dispPtr->dynClkAddr);
	ClkStart(_dynclk_mm_IP, dispPtr->dynClkAddr);
	...
	XVtc_SelfTest(_vtc_mm_IP, &(dispPtr->vtc));

	XVtc_RegUpdateEnable(_vtc_mm_IP, &(dispPtr->vtc));
	XVtc_SetGeneratorTiming(_vtc_mm_IP,&(dispPtr->vtc), &vtcTiming);
	XVtc_SetSource(_vtc_mm_IP, &(dispPtr->vtc), &SourceSelect);
    /*
	 * Enable VTC core, releasing backpressure on VDMA
	 */
	XVtc_EnableGenerator(_vtc_mm_IP, &dispPtr->vtc);
	...
	Status = XAxiVdma_DmaConfig(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ, &(dispPtr->vdmaConfig));
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Read channel config failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_DmaSetBufferAddr(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ, dispPtr->vdmaConfig.FrameStoreStartAddr);
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Read channel set buffer address failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_DmaStart(_vdma_mm_IP, dispPtr->vdma, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Start read transfer failed %d\r\n", Status);
		return XST_FAILURE;
	}
	Status = XAxiVdma_StartParking(_vdma_mm_IP, dispPtr->vdma, dispPtr->curFrame, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
...
**
*/
int DisplayInitialize(mm_IP * _vtc_mm_IP, mm_IP * _dynclk_mm_IP,DisplayCtrl *dispPtr, XAxiVdma *vdma, u16 vtcId, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
	int Status;
	int i;
	...
	dispPtr->state = DISPLAY_STOPPED;
	dispPtr->stride = dispPtr->vMode.width *4 ;
    ...
    */
	...
	// initialize the vtc_mm_IP
	_vtc_mm_IP->base_address = vtcConfig->BaseAddress;
    _vtc_mm_IP->size_in_k = 64;
    create_IP_driver(_vtc_mm_IP);


	Status = XVtc_CfgInitialize(_vtc_mm_IP, &(dispPtr->vtc), vtcConfig, vtcConfig->BaseAddress);
	/* Checking status */
	if (Status != (XST_SUCCESS)) {
		return (XST_FAILURE);
	}
...
**
*/
int DisplaySetMode(mm_IP * _vdma_mm_IP ,mm_IP * _vtc_mm_IP , DisplayCtrl *dispPtr, const VideoMode *newMode)
{
	int Status;
	...
	if (dispPtr->state == DISPLAY_RUNNING)
	{
		Status = DisplayStop(_vdma_mm_IP,_vtc_mm_IP, dispPtr);
		if (Status != XST_SUCCESS)
		{
...
int DisplayChangeFrame(mm_IP * _mm_IP,DisplayCtrl *dispPtr, u32 frameIndex)
{
	int Status;
	...
	if (dispPtr->state == DISPLAY_RUNNING)
	{
		Status = XAxiVdma_StartParking(_mm_IP, dispPtr->vdma, dispPtr->curFrame, XAXIVDMA_READ);
		if (Status != XST_SUCCESS)
		{
