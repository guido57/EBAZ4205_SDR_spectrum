    ...
    switch (key) {
		...
		case 'r':
			if(strcmp(state->argv[state->next],"640x480")==0)
				...
				dispCtrl.vMode = VMODE_1280x720x50Hz;
				...
				dispCtrl.vMode = VMODE_1920x1080x60Hz;
			else
				dispCtrl.vMode = VMODE_1280x720x50Hz;
			break;
...
void DemoChangeRes()
{
	...
	while (!fResSet)
	{
		...
		case '5':
			status = DisplayStop(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl);
			DisplaySetMode(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl, &VMODE_1280x720x45Hz);
			DisplayStart(&vtc_mm_IP,&vdma_mm_IP, &dynclk_mm_IP,&dispCtrl);
			fResSet = 1;
			break;
		...
		case '6':
			status = DisplayStop(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl);
			DisplaySetMode(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl, &VMODE_1280x720x30Hz);
			DisplayStart(&vtc_mm_IP,&vdma_mm_IP, &dynclk_mm_IP,&dispCtrl);
			fResSet = 1;
			break;
		...
		case '7':
			status = DisplayStop(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl);
			DisplaySetMode(&vdma_mm_IP, &vtc_mm_IP,&dispCtrl, &VMODE_1280x720x25Hz);
			DisplayStart(&vtc_mm_IP,&vdma_mm_IP, &dynclk_mm_IP,&dispCtrl);
			fResSet = 1;
			break;
