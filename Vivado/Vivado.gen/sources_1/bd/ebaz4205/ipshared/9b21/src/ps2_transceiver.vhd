...
BEGIN
  ...
  BEGIN
    IF(clk'EVENT AND clk = '1') THEN  --rising edge of system clock
      ...
      sync_ffs(0) <= ps2_clock_I;          --synchronize PS/2 clock signal
      sync_ffs(1) <= ps2_dat_I;            --synchronize PS/2 data signal
    END IF;
  END PROCESS;
  ...
  BEGIN
    IF(reset_n = '0') THEN                  --reset PS/2 transceiver
      ...
      ps2_clock_T <= '0';                       -- enable clock out    
      ps2_clock_O <= '0';                       --inhibit communication on PS/2 bus
      ps2_dat_T <= '1';                        --release PS/2 data line
      --GG tx_busy <= '1';                         --indicate that no transmit is in progress
      tx_busy <= '0';                         --indicate that no transmit is in progress
      ...
      CASE state IS                           --implement state machine
        ...
        WHEN receive =>
          ...
          ELSE                                                   --transmit not requested
            ...
            ps2_clock_T <= '1';                                        --release PS/2 clock port
            ps2_dat_T <= '1';                                       --release PS/2 data port    
        ...
        WHEN inhibit =>
          IF(timer < clk_freq/10_000) THEN     --first 100us not complete
            ...
            ps2_dat_T <= '1';                     --release data port
            ps2_clock_T <= '0';                      --enable clock out
            ps2_clock_O <= '0';                      --inhibit communication
            state <= inhibit;                    --continue inhibit
          ELSE                                 --100us complete
            --ps2_data <= ps2_word(0);             --output start bit to PS/2 data port
            ps2_dat_T <= '0';                     -- enable data out    
            ps2_dat_O <= ps2_word(0);             --output start bit to PS/2 data port
            state <= transact;                   --proceed to send bits
          END IF;
          
        WHEN transact =>
          --ps2_clk <= 'Z';                                       --release clock port
          ps2_clock_T <= '1';                                   --release clock port
          IF(ps2_clk_int_prev = '1' AND ps2_clk_int = '0') THEN --falling edge of PS2 clock
            ps2_word <= ps2_data_int & ps2_word(10 DOWNTO 1);     --shift contents of PS/2 data buffer
          ...
          IF(bit_cnt < 10) THEN                                 --all bits not sent
            ...
            ps2_dat_T <= '0';                                       -- enable data out    
            ps2_dat_O <= ps2_word(0);                              --connect serial output of PS/2 data buffer to data port
          ELSE                                                  --all bits sent
            --ps2_data <= 'Z';                                      --release data port
            ps2_dat_T <= '1';                                      --release data port
          END IF;
