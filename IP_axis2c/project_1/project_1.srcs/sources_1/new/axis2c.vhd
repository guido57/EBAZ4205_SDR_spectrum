...
begin
  ...
  process(m_axis_tready) begin
    if rising_edge(m_axis_tready) then
        m_axis_tvalid <= '1';
        m_axis_tlast <= tlast_reg;
  end if;
    if falling_edge(m_axis_tready) then
        m_axis_tvalid <= '0';
        m_axis_tlast <= '0';
    end if;
  end process;
  ...
  process(aclk) begin
    if rising_edge(aclk) then
      if aresetn /= '1' then
        ...
        m_axis_tvalid_reg <= '0';
        tlast_reg <= '0';
      elsif tlast_reg = '1' and m_axis_tready = '1' then
        -- send out data0
        m_axis_tdata <= s_axis_tdata0;
        tlast_reg <= '0';
      elsif tlast_reg = '0' and m_axis_tready = '1' then  
        -- send out data1
