...
WideGraph::WideGraph(QSettings * settings, JTDXDateTime * jtdxtime, QWidget *parent) :
  ...
  ui(new Ui::WideGraph),
  m_settings (settings),
  //GG m_palettes_path {"Palettes"},
  m_palettes_path {QDir::homePath() + "/.local/share/JTDX/Palettes/"},
  m_tr0 {0.0},
  m_lockTxFreq {false},
  m_filter {false},
  m_jtdxtime {jtdxtime}
{
  ...
  connect(ui->pbShowWSJTX,&QPushButton::clicked,this,&WideGraph::showWSJTX);
...
void WideGraph::showWSJTX()
{
  hide();
  MainWindow * mw = (MainWindow *) this->parent();
  mw->ui->actionWide_WSJT_X->setChecked(true);
}
