...
//  m_olek2 {false},
  ...
  //m_wideGraph (new WideGraph(settings, m_jtdxtime, this)),
...
//		else { ui->stopTxButton->click(); }
  ...
  // init the logbook to populate countries with cty.dat
  m_logBook.init(m_config.callNotif() ? m_config.my_callsign() : "",m_config.gridNotif() ? m_config.my_grid() : "",m_config.timeFrom());

  connect(ui->actionWide_SDRApp, &QAction::triggered, this, &MainWindow::on_actionWide_SDRApp_triggered);
  connect(ui->actionWide_WSJT_X, &QAction::triggered, this, &MainWindow::on_actionWide_WSJTX_triggered);
  connect(ui->actionWide_Waterfall, &QAction::triggered, this, &MainWindow::on_actionWide_Waterfall_triggered);

  QActionGroup* viewGroup = new QActionGroup(this);
  ui->actionWide_SDRApp->setActionGroup(viewGroup);
  ui->actionWide_WSJT_X->setActionGroup(viewGroup);
  ui->actionWide_Waterfall->setActionGroup(viewGroup);

  // always start as WSJT-X
  ui->actionWide_WSJT_X->setChecked(true);
  on_actionWide_WSJTX_triggered();
...
//  setWindowTitle (program_title ());
  ...
  //GG TEMPORARY connect(&proc_jtdxjt9, &QProcess::readyReadStandardOutput,this, &MainWindow::readFromStdout);
  ...
  // Connect for the WSPR decoder process (p1)
  ...
  connect(&proc_jt9, static_cast<void (QProcess::*) (int, QProcess::ExitStatus)> (&QProcess::finished),
          [this] (int exitCode, QProcess::ExitStatus status) {
              ...
              if (subProcessFailed (&proc_jt9, exitCode, status))
              {
                  m_valid = false;          // ensures exit if still
                      // constructing
                  //GG QTimer::singleShot (0, this, SLOT (close ()));
              }
          });
  ...
  // hook up save WAV file exit handling to call the external FT8 decoder by call_jt9
...
//      , "-m", QString::number (qMin (qMax (QThread::idealThreadCount (), 1), 24)) //FFTW threads
  ...
  //GG proc_jtdxjt9.start(QDir::toNativeSeparators (m_appDir) + QDir::separator () +
  //GG        "jtdxjt9", jt9_args, QIODevice::ReadWrite | QIODevice::Unbuffered);

  // Retrieve the FFTW parameters (wisdom) from file
...
//  else Q_EMIT transmitFrequency (ui->TxFreqSpinBox->value () - m_XIT);
  ...
  //GG on_actionWide_Waterfall_triggered();
...
void MainWindow::readSettings()
{
  ...
  //GG m_geometry = m_settings->value ("geometry",saveGeometry()).toByteArray();
  //GG restoreGeometry(m_geometry);
  //GG m_geometry = saveGeometry(); //GG save geometry (full screen) to m_geometry
  setGeometry(0,0,1280,720);   //GG full screen
...
void MainWindow::dataSink(qint64 frames)
{
  ...
  //qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss:zzz") << " datasink: frames=" << frames;

  //GG tell dec_data (shared data between this program and jtdxjt9 program) if we are decoding from a wav file
  if(m_diskData) dec_data.params.ndiskdat=1; else dec_data.params.ndiskdat=0;

  //GG Get power (0-60 dB), spectrum, and ihsym using symspec.f90
  ...
  if(ihsym <=0)
    return;
  //qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss:zzz") << " GetOffset=" << m_jtdxtime->GetOffset() << " last=" << last.toString("hh:mm:ss.zzz") << " ihsym=" << ihsym << " k=" <<k;
  QString t;
...
//    m_dateTime = now.toString ("yyyy-MMM-dd hh:mm");
    ...
    //GG TEMPORARY
    //GG if(!m_decoderBusy && m_mode.left(4)!="WSPR") { //Start decoder
    //  last=now; decode();
    ...
    QString wavfilenameonly;

    // Save the received data as a .wav file
    if(!m_diskData && (m_saveWav!=0 ||
              m_mode.mid (0,4) == "WSPR" ||
              m_mode.mid (0,3) == "FT8"
                      )
      ...
      if(m_mode.startsWith("FT")) {
        ...
        wavfilenameonly = period_start.toString("yyMMdd_hhmmss");
        m_fnameWE=m_config.save_directory().absoluteFilePath (wavfilenameonly);
      } else {
    ...
    // Create the command line to start the WSPR decoder process (wsprd_jtdx)
...
//      cmnd=t3.left(i1+7) + t3.mid(i1+7);
      ...
      // Start the WSPR decoder process (wsprd_jtdx)
...
QString MainWindow::save_wave_file (QString const& name, short const * data, int samples,
        QString const& my_callsign, QString const& my_grid, QString const& mode,
                                   Frequency frequency, QString const& his_call, QString const& his_grid,JTDXDateTime * jtdxtime)
{
  //
...
void MainWindow::call_jt9 (QString  fname) {
  //-------------------------------------------------------------------------------------
  //GG Added by IW5ALZ to use an FT8 decoder (jt9) to read from wav file instead from the shared memory
  //-------------------------------------------------------------------------------------
  // Create the command line to start the FT8 decoder process (jt9)
  QStringList cmnd_jt9;
  QString depth_string;
  const QFileInfo info(fname);
  const QString wavfilenameonly(info.fileName());
...
void MainWindow::on_actionWide_Waterfall_triggered() {
  ...
  ui->statusBar->hide();
  m_wideGraph->show();
  //m_wideGraph->setGeometry(0,0,1280,720);
} //Display Waterfalls

void MainWindow::on_actionWide_SDRApp_triggered() {
  m_wideGraph->hide();
  ui->menuBar->show();
  ui->statusBar->show();
  ui->centralWidget->show();
  QRect geom = m_wideGraph->geometry();
  //setGeometry(0,0,1280,720);
} //Display SDR-App

void MainWindow::on_actionWide_WSJTX_triggered() {
  m_wideGraph->hide();
  ui->centralWidget->show();
  ui->menuBar->show();
  ui->statusBar->show();
...
//  printf("%s(%0.1f) Timing decode stop, %d candidates\n",m_jtdxtime->currentDateTimeUtc2().toString("hh:mm:ss.zzz").toStdString().c_str(),m_jtdxtime->GetOffset(),m_ncand);
      ...
      if(t.indexOf(m_baseCall.toLatin1()) >= 0 || m_config.write_decoded() || m_config.write_decoded_debug()) {
        // Append the decoded row to the monthly log. E.g. /home/ebaz/.local/share/JTDX/2023_05.ALL.TXT
        QFile f {m_dataDir.absoluteFilePath (m_jtdxtime->currentDateTimeUtc2().toString("yyyyMM_")+"ALL.TXT")};
        if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
          QTextStream out(&f);
          if (m_RxLog==1) {
            // Write Date Time Freq and Mode if enabled ...
      ...
      {
          // Insert a line spacer  with some info on the ui->decodedTextBrowser
          QString band;
      ...
      // Create a DecodedText object from the decoded string to interpret its data
...
//GG proc_jt9 is the FT8 decoder process. started by MainWindow:DataSink
//GG this function is identical to ReadFromStdOut except it reads from jt9 and not from jtdxjt9
void MainWindow::proc_jt9_ReadFromStdout()                             //readFromStdout
{

  while(proc_jt9.canReadLine()) {

    QByteArray t=proc_jt9.readLine();
    //printf("proc_jt9_ReadFromStdout received: %s\r\n", QString(t).toStdString().c_str());
    if(t.startsWith("<DecodeFinished>")) {
      dec_data.params.nstophint=1;
      m_bDecoded = t.mid (20).trimmed ().toInt () > 0;
      int mswait=750.0*m_TRperiod;
      ...
      // qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss:zzz") << ": decode started";
      if(t.indexOf(m_baseCall.toLatin1()) >= 0 || m_config.write_decoded() || m_config.write_decoded_debug()) {
          // Append the decoded row to the monthly log. E.g. /home/ebaz/.local/share/JTDX/2023_05_ALL.TXT
          QFile f {m_dataDir.absoluteFilePath (m_jtdxtime->currentDateTimeUtc2().toString("yyyyMM_")+"ALL.TXT")};
          if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
            QTextStream out(&f);
            if (m_RxLog==1) {
...
// During decoding, a few buttons are disabled
void MainWindow::decodeBusy(bool b)                             //decodeBusy()
{
  ...
  //GG ui->actionOpen->setEnabled(!b);
  ui->actionOpen->setEnabled(true); //GG
...
// Answer a CQ when the row is double clicked
...
void MainWindow::on_outAttenuation_valueChanged (int a)
{
  ...
  //GG if (!m_block_pwr_tooltip) QToolTip::showText (QCursor::pos (), tt_str, ui->outAttenuation);
  if (m_mode == "JT9+JT65" && m_modeTx == "JT65") { curBand = ui->bandComboBox->currentText()+m_modeTx; }
  else { curBand = ui->bandComboBox->currentText()+m_mode; }
...
// p1 is the WSPR decoder process. started by MainWindow:DataSink when WSPR is enabled
