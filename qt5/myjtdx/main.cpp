...
int main(int argc, char *argv[])
{
  ...
  // On arm (PetaLinux) start with the linux framebuffer driver (LINUXFB)
  ...
  //GG ExceptionCatchingApplication a(argc, argv);
  ...
  if (version().replace("_32A","").indexOf("_") > 1) {
    ...
    if (QDate().currentDate() > expire_date) {
...
#endif
        printf("ConfigLocation=%s\r\n", QStandardPaths::writableLocation(QStandardPaths::ConfigLocation).toStdString ().c_str());
        printf("TempLocation=%s\r\n", QStandardPaths::writableLocation(QStandardPaths::TempLocation).toStdString ().c_str());
        printf("AppConfigLocation=%s\r\n", QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation).toStdString ().c_str());
        printf("DataLocation=%s\r\n", QStandardPaths::writableLocation(QStandardPaths::DataLocation).toStdString ().c_str());
        printf("RuntimeLocation=%s\r\n", QStandardPaths::writableLocation(QStandardPaths::RuntimeLocation).toStdString ().c_str());
        QString hl = QStandardPaths::writableLocation(QStandardPaths::HomeLocation);
        printf("HomeLocation=%s\r\n", hl.toStdString ().c_str());
      ...
      if (!config_path.mkpath ("."))
        {
            config_path.setPath(QDir::currentPath());
