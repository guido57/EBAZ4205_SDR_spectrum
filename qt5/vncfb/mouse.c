...
int init_mouse(const char *touch_device, int vnc_rotate)
{
    ...
    if (ioctl(mousefd, EVIOCGABS(ABS_X), &info))
    {
        xmin = 0;
        xmax = 1279;
        error_print("cannot get ABS_X info, %s   Set xmin=%d  xmax=%d\n", strerror(errno), xmin,xmax);
    }else{
        xmin = info.minimum;
        xmax = info.maximum;
    }
    ...
    if (ioctl(mousefd, EVIOCGABS(ABS_Y), &info))
    {
        ymin = 0;
        ymax = 719;
        error_print("cannot get ABS_Y, %s   Set ymin=%d  ymax=%d\n", strerror(errno), ymin, ymax);
    }else{
        ymin = info.minimum;
        ymax = info.maximum;
    }
...
void injectMouseEvent(struct fb_var_screeninfo *scrinfo, int buttonMask, int x, int y)
{
    ...
    if(verbose)
        info_print("injectMouseEvent (screen(%d,%d) -> mouse(%d,%d), button=%d, wheel tick=%d)\n", xin, yin, x, y, buttonMask, last_wheel_tick);
}
