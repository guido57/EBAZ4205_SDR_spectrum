...
static char KBD_DEVICE[256] = "/dev/uinput";
static int kbdfd = -1;

int init_kbd_ori(const char *kbd_device)
{
...
int init_kbd(const char *kbd_device)
{
    ...
    /*
    * The ioctls below will enable the uinput device that is about to be
    * created, to pass key events, in this case the space key.
    */
    struct uinput_setup usetup;

    int ret = ioctl(kbdfd, UI_SET_EVBIT, EV_KEY);
    if (ret < 0)
    {
        error_print("ioctl: UI_SET_EVBIT, EV_KEY failed, %s\n", strerror(errno));
        return 0;
    }

    for (int key=1; key<=148;key++){

        ioctl(kbdfd, UI_SET_KEYBIT, key);
    ...
    ret = ioctl(kbdfd, UI_DEV_SETUP, &usetup);
    ret = ioctl(kbdfd, UI_DEV_CREATE);

    //sleep(3);

    //init_kbd_ori("/dev/input/event1");
...
void injectKeyEvent(uint16_t code, uint16_t value)
{
    ...
    if (write(kbdfd, &ev, sizeof(ev)) < 0)
    {
        error_print("write event failed sending SYN, %s\n", strerror(errno));
    }

    info_print("injectKey (%d, %d)\n", code, value);
}
...
int keysym2scancode(rfbKeySym key, rfbClientPtr cl)
{
    ...
    else
    {
        ...
        case XK_slash:          scancode = KEY_KPSLASH;   break;  //2f
        // XK_0 .. XK_9         // 30 .. 39
